...
fn main() -> io::Result<()> {
    ...
    let buffer_size = std::mem::size_of::<i32>() + std::mem::size_of::<usize>() + config.data_size() + std::mem::size_of::<u16>();
    ...
    loop {
        ...
        crc_ccitt.digest(&buffer[..(buffer_size - std::mem::size_of::<u16>())]); // 计算校验码
        let crc = crc_ccitt.get_crc(); // 保存校验码
        crc_ccitt.reset(); // 重置crc状态

        if recv_checksum != crc_ccitt.get_crc() {
            println!("校验码出错(期待的: {}, 实际收到的: {}), 丢弃", crc, recv_checksum);
            if seq_num == 0 {
                continue;
            }
            let ack_data = bincode::serialize(&Ack::new(seq_num - 1)).unwrap();
